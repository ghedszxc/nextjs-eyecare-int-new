 ...
 */
export function buildBlogSearchLayout(
  values: unknown,
  options: BlogSearchLayoutOptions = {},
): ILayoutModel {
  const { before = [], after = [], title = "", description = "" } = options;

  const blogSearchResultWidget: IWidgetModel = {
  ...
  return {
    layoutType: "BlogSearchLayout",
    widgets: [...before, blogSearchResultWidget, ...after],
    title,
    description,
  };
}
