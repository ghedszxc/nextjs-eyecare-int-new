...
type GlobalSearchLayoutOptions = {
 ...
 */
export function buildGlobalSearchLayout(
  values: unknown,
  options: GlobalSearchLayoutOptions = {},
): ILayoutModel {
  const { before = [], after = [], title = "", description = "" } = options;

  const globalSearchResultWidget: IWidgetModel = {
    widgetName: "global-search-results-list",
    // Mirror the CMS shape: adapters read widgetValue as an array of items.
    widgetValue: [values] as IWidgetModel["widgetValue"],
  };

  return {
    layoutType: "globalSearchResultList",
    widgets: [...before, globalSearchResultWidget, ...after],
    title,
    description,
  };
}
